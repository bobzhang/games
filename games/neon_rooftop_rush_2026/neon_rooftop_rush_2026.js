...
var ASM_CONSTS = {
  268232: () => { if (document.fullscreenElement) return 1; },  
 268278: () => { return document.getElementById('canvas').width; },  
 268330: () => { return parseInt(document.getElementById('canvas').style.width); },  
 268398: () => { document.exitFullscreen(); },  
 268425: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 268498: () => { if (document.fullscreenElement) return 1; },  
 268544: () => { return document.getElementById('canvas').width; },  
 268596: () => { return screen.width; },  
 268621: () => { document.exitFullscreen(); },  
 268648: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 268781: () => { return window.innerWidth; },  
 268807: () => { return window.innerHeight; },  
 268834: () => { if (document.fullscreenElement) return 1; },  
 268880: () => { return document.getElementById('canvas').width; },  
 268932: () => { return parseInt(document.getElementById('canvas').style.width); },  
 269000: () => { if (document.fullscreenElement) return 1; },  
 269046: () => { return document.getElementById('canvas').width; },  
 269098: () => { return screen.width; },  
 269123: () => { return window.innerWidth; },  
 269149: () => { return window.innerHeight; },  
 269176: () => { if (document.fullscreenElement) return 1; },  
 269222: () => { return document.getElementById('canvas').width; },  
 269274: () => { return screen.width; },  
 269299: () => { document.exitFullscreen(); },  
 269326: () => { if (document.fullscreenElement) return 1; },  
 269372: () => { return document.getElementById('canvas').width; },  
 269424: () => { return parseInt(document.getElementById('canvas').style.width); },  
 269492: () => { document.exitFullscreen(); },  
 269519: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 269577: () => { return screen.width; },  
 269602: () => { return screen.height; },  
 269628: () => { return window.screenX; },  
 269655: () => { return window.screenY; },  
 269682: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 269735: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 269806: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 269863: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 270119: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 270190: () => { if (document.fullscreenElement) return 1; },  
 270236: () => { return window.innerWidth; },  
 270262: () => { return window.innerHeight; },  
 270289: () => { if (document.pointerLockElement) return 1; },  
 270336: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 272494: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 272658: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 272762: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 275761: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 275827: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 276220: ($0) => { miniaudio.untrack_device_by_index($0); },  
 276263: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 276388: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
