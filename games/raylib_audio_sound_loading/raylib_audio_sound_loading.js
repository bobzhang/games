...
function findWasmBinary() {
  return locateFile('raylib_audio_sound_loading.wasm');
}
...
var ASM_CONSTS = {
  240432: () => { if (document.fullscreenElement) return 1; },  
 240478: () => { return document.getElementById('canvas').width; },  
 240530: () => { return parseInt(document.getElementById('canvas').style.width); },  
 240598: () => { document.exitFullscreen(); },  
 240625: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 240698: () => { if (document.fullscreenElement) return 1; },  
 240744: () => { return document.getElementById('canvas').width; },  
 240796: () => { return screen.width; },  
 240821: () => { document.exitFullscreen(); },  
 240848: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 240981: () => { return window.innerWidth; },  
 241007: () => { return window.innerHeight; },  
 241034: () => { if (document.fullscreenElement) return 1; },  
 241080: () => { return document.getElementById('canvas').width; },  
 241132: () => { return parseInt(document.getElementById('canvas').style.width); },  
 241200: () => { if (document.fullscreenElement) return 1; },  
 241246: () => { return document.getElementById('canvas').width; },  
 241298: () => { return screen.width; },  
 241323: () => { return window.innerWidth; },  
 241349: () => { return window.innerHeight; },  
 241376: () => { if (document.fullscreenElement) return 1; },  
 241422: () => { return document.getElementById('canvas').width; },  
 241474: () => { return screen.width; },  
 241499: () => { document.exitFullscreen(); },  
 241526: () => { if (document.fullscreenElement) return 1; },  
 241572: () => { return document.getElementById('canvas').width; },  
 241624: () => { return parseInt(document.getElementById('canvas').style.width); },  
 241692: () => { document.exitFullscreen(); },  
 241719: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 241777: () => { return screen.width; },  
 241802: () => { return screen.height; },  
 241828: () => { return window.screenX; },  
 241855: () => { return window.screenY; },  
 241882: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 241935: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 242006: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 242063: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 242319: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 242390: () => { if (document.fullscreenElement) return 1; },  
 242436: () => { return window.innerWidth; },  
 242462: () => { return window.innerHeight; },  
 242489: () => { if (document.pointerLockElement) return 1; },  
 242536: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 244694: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 244858: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 244962: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 247961: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 248027: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 248420: ($0) => { miniaudio.untrack_device_by_index($0); },  
 248463: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 248588: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
