...
var ASM_CONSTS = {
  314284: () => { if (document.fullscreenElement) return 1; },  
 314330: () => { return document.getElementById('canvas').width; },  
 314382: () => { return parseInt(document.getElementById('canvas').style.width); },  
 314450: () => { document.exitFullscreen(); },  
 314477: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 314550: () => { if (document.fullscreenElement) return 1; },  
 314596: () => { return document.getElementById('canvas').width; },  
 314648: () => { return screen.width; },  
 314673: () => { document.exitFullscreen(); },  
 314700: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 314833: () => { return window.innerWidth; },  
 314859: () => { return window.innerHeight; },  
 314886: () => { if (document.fullscreenElement) return 1; },  
 314932: () => { return document.getElementById('canvas').width; },  
 314984: () => { return parseInt(document.getElementById('canvas').style.width); },  
 315052: () => { if (document.fullscreenElement) return 1; },  
 315098: () => { return document.getElementById('canvas').width; },  
 315150: () => { return screen.width; },  
 315175: () => { return window.innerWidth; },  
 315201: () => { return window.innerHeight; },  
 315228: () => { if (document.fullscreenElement) return 1; },  
 315274: () => { return document.getElementById('canvas').width; },  
 315326: () => { return screen.width; },  
 315351: () => { document.exitFullscreen(); },  
 315378: () => { if (document.fullscreenElement) return 1; },  
 315424: () => { return document.getElementById('canvas').width; },  
 315476: () => { return parseInt(document.getElementById('canvas').style.width); },  
 315544: () => { document.exitFullscreen(); },  
 315571: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 315629: () => { return screen.width; },  
 315654: () => { return screen.height; },  
 315680: () => { return window.screenX; },  
 315707: () => { return window.screenY; },  
 315734: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 315787: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 315858: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 315915: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 316171: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 316242: () => { if (document.fullscreenElement) return 1; },  
 316288: () => { return window.innerWidth; },  
 316314: () => { return window.innerHeight; },  
 316341: () => { if (document.pointerLockElement) return 1; },  
 316388: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 318546: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 318710: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 318814: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 321813: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 321879: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 322272: ($0) => { miniaudio.untrack_device_by_index($0); },  
 322315: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 322440: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
