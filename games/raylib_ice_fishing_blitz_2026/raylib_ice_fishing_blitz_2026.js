...
var ASM_CONSTS = {
  107672: () => { if (document.fullscreenElement) return 1; },  
 107718: () => { return document.getElementById('canvas').width; },  
 107770: () => { return parseInt(document.getElementById('canvas').style.width); },  
 107838: () => { document.exitFullscreen(); },  
 107865: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 107938: () => { if (document.fullscreenElement) return 1; },  
 107984: () => { return document.getElementById('canvas').width; },  
 108036: () => { return screen.width; },  
 108061: () => { document.exitFullscreen(); },  
 108088: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 108221: () => { return window.innerWidth; },  
 108247: () => { return window.innerHeight; },  
 108274: () => { if (document.fullscreenElement) return 1; },  
 108320: () => { return document.getElementById('canvas').width; },  
 108372: () => { return parseInt(document.getElementById('canvas').style.width); },  
 108440: () => { if (document.fullscreenElement) return 1; },  
 108486: () => { return document.getElementById('canvas').width; },  
 108538: () => { return screen.width; },  
 108563: () => { return window.innerWidth; },  
 108589: () => { return window.innerHeight; },  
 108616: () => { if (document.fullscreenElement) return 1; },  
 108662: () => { return document.getElementById('canvas').width; },  
 108714: () => { return screen.width; },  
 108739: () => { document.exitFullscreen(); },  
 108766: () => { if (document.fullscreenElement) return 1; },  
 108812: () => { return document.getElementById('canvas').width; },  
 108864: () => { return parseInt(document.getElementById('canvas').style.width); },  
 108932: () => { document.exitFullscreen(); },  
 108959: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 109017: () => { return screen.width; },  
 109042: () => { return screen.height; },  
 109068: () => { return window.screenX; },  
 109095: () => { return window.screenY; },  
 109122: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 109175: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 109246: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 109303: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 109559: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 109630: () => { if (document.fullscreenElement) return 1; },  
 109676: () => { return window.innerWidth; },  
 109702: () => { return window.innerHeight; },  
 109729: () => { if (document.pointerLockElement) return 1; },  
 109776: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 111934: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 112098: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 112202: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 115201: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 115267: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 115660: ($0) => { miniaudio.untrack_device_by_index($0); },  
 115703: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 115828: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
