...
function findWasmBinary() {
  return locateFile('raylib_audio_raw_stream.wasm');
}
...
var ASM_CONSTS = {
  105672: () => { if (document.fullscreenElement) return 1; },  
 105718: () => { return document.getElementById('canvas').width; },  
 105770: () => { return parseInt(document.getElementById('canvas').style.width); },  
 105838: () => { document.exitFullscreen(); },  
 105865: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 105938: () => { if (document.fullscreenElement) return 1; },  
 105984: () => { return document.getElementById('canvas').width; },  
 106036: () => { return screen.width; },  
 106061: () => { document.exitFullscreen(); },  
 106088: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 106221: () => { return window.innerWidth; },  
 106247: () => { return window.innerHeight; },  
 106274: () => { if (document.fullscreenElement) return 1; },  
 106320: () => { return document.getElementById('canvas').width; },  
 106372: () => { return parseInt(document.getElementById('canvas').style.width); },  
 106440: () => { if (document.fullscreenElement) return 1; },  
 106486: () => { return document.getElementById('canvas').width; },  
 106538: () => { return screen.width; },  
 106563: () => { return window.innerWidth; },  
 106589: () => { return window.innerHeight; },  
 106616: () => { if (document.fullscreenElement) return 1; },  
 106662: () => { return document.getElementById('canvas').width; },  
 106714: () => { return screen.width; },  
 106739: () => { document.exitFullscreen(); },  
 106766: () => { if (document.fullscreenElement) return 1; },  
 106812: () => { return document.getElementById('canvas').width; },  
 106864: () => { return parseInt(document.getElementById('canvas').style.width); },  
 106932: () => { document.exitFullscreen(); },  
 106959: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 107017: () => { return screen.width; },  
 107042: () => { return screen.height; },  
 107068: () => { return window.screenX; },  
 107095: () => { return window.screenY; },  
 107122: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 107175: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 107246: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 107303: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 107559: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 107630: () => { if (document.fullscreenElement) return 1; },  
 107676: () => { return window.innerWidth; },  
 107702: () => { return window.innerHeight; },  
 107729: () => { if (document.pointerLockElement) return 1; },  
 107776: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 109934: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 110098: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 110202: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 113201: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 ...
 113660: ($0) => { miniaudio.untrack_device_by_index($0); },  
 113703: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 113828: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
