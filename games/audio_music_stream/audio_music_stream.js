...
var ASM_CONSTS = {
  1785884: () => { if (document.fullscreenElement) return 1; },  
 1785930: () => { return document.getElementById('canvas').width; },  
 1785982: () => { return parseInt(document.getElementById('canvas').style.width); },  
 1786050: () => { document.exitFullscreen(); },  
 1786077: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 1786150: () => { if (document.fullscreenElement) return 1; },  
 1786196: () => { return document.getElementById('canvas').width; },  
 1786248: () => { return screen.width; },  
 1786273: () => { document.exitFullscreen(); },  
 1786300: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 1786433: () => { return window.innerWidth; },  
 1786459: () => { return window.innerHeight; },  
 1786486: () => { if (document.fullscreenElement) return 1; },  
 1786532: () => { return document.getElementById('canvas').width; },  
 1786584: () => { return parseInt(document.getElementById('canvas').style.width); },  
 1786652: () => { if (document.fullscreenElement) return 1; },  
 1786698: () => { return document.getElementById('canvas').width; },  
 1786750: () => { return screen.width; },  
 1786775: () => { return window.innerWidth; },  
 1786801: () => { return window.innerHeight; },  
 1786828: () => { if (document.fullscreenElement) return 1; },  
 1786874: () => { return document.getElementById('canvas').width; },  
 1786926: () => { return screen.width; },  
 1786951: () => { document.exitFullscreen(); },  
 1786978: () => { if (document.fullscreenElement) return 1; },  
 1787024: () => { return document.getElementById('canvas').width; },  
 1787076: () => { return parseInt(document.getElementById('canvas').style.width); },  
 1787144: () => { document.exitFullscreen(); },  
 1787171: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 1787229: () => { return screen.width; },  
 1787254: () => { return screen.height; },  
 1787280: () => { return window.screenX; },  
 1787307: () => { return window.screenY; },  
 1787334: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 1787387: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 1787458: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 1787515: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 1787771: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 1787842: () => { if (document.fullscreenElement) return 1; },  
 1787888: () => { return window.innerWidth; },  
 1787914: () => { return window.innerHeight; },  
 1787941: () => { if (document.pointerLockElement) return 1; },  
 1787988: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 1790146: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 1790310: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 1790414: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 1793413: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 1793479: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 1793872: ($0) => { miniaudio.untrack_device_by_index($0); },  
 1793915: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 1794040: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
