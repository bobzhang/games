...
var ASM_CONSTS = {
  269208: () => { if (document.fullscreenElement) return 1; },  
 269254: () => { return document.getElementById('canvas').width; },  
 269306: () => { return parseInt(document.getElementById('canvas').style.width); },  
 269374: () => { document.exitFullscreen(); },  
 269401: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 269474: () => { if (document.fullscreenElement) return 1; },  
 269520: () => { return document.getElementById('canvas').width; },  
 269572: () => { return screen.width; },  
 269597: () => { document.exitFullscreen(); },  
 269624: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 269757: () => { return window.innerWidth; },  
 269783: () => { return window.innerHeight; },  
 269810: () => { if (document.fullscreenElement) return 1; },  
 269856: () => { return document.getElementById('canvas').width; },  
 269908: () => { return parseInt(document.getElementById('canvas').style.width); },  
 269976: () => { if (document.fullscreenElement) return 1; },  
 270022: () => { return document.getElementById('canvas').width; },  
 270074: () => { return screen.width; },  
 270099: () => { return window.innerWidth; },  
 270125: () => { return window.innerHeight; },  
 ...
 270275: () => { document.exitFullscreen(); },  
 270302: () => { if (document.fullscreenElement) return 1; },  
 270348: () => { return document.getElementById('canvas').width; },  
 270400: () => { return parseInt(document.getElementById('canvas').style.width); },  
 270468: () => { document.exitFullscreen(); },  
 270495: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 270553: () => { return screen.width; },  
 270578: () => { return screen.height; },  
 270604: () => { return window.screenX; },  
 270631: () => { return window.screenY; },  
 270658: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 270711: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 270782: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 270839: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 271095: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 271166: () => { if (document.fullscreenElement) return 1; },  
 271212: () => { return window.innerWidth; },  
 271238: () => { return window.innerHeight; },  
 271265: () => { if (document.pointerLockElement) return 1; },  
 271312: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 273470: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 273634: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 273738: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 276737: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 276803: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 277196: ($0) => { miniaudio.untrack_device_by_index($0); },  
 277239: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 277364: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
